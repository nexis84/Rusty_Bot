...
// Material inventory persistence
const getStoredMaterials = (): Record<string, number> => {
  const userId = getUserId();
  const stored = localStorage.getItem(`sleeper_materials_${userId}`);
  return stored ? JSON.parse(stored) : {};
};

const storeMaterials = (materials: Record<string, number>): void => {
  const userId = getUserId();
  localStorage.setItem(`sleeper_materials_${userId}`, JSON.stringify(materials));
};

// Blueprint inventory persistence
const getStoredOwnedBlueprints = (): string[] => {
  const userId = getUserId();
  const stored = localStorage.getItem(`sleeper_blueprints_${userId}`);
  return stored ? JSON.parse(stored) : [];
};

const storeBlueprint = (blueprintId: string): void => {
  const userId = getUserId();
  const bps = getStoredOwnedBlueprints();
  if (!bps.includes(blueprintId)) {
    bps.push(blueprintId);
    localStorage.setItem(`sleeper_blueprints_${userId}`, JSON.stringify(bps));
  }
};

// Implant inventory persistence
const getStoredOwnedImplants = (): string[] => {
  const userId = getUserId();
  const stored = localStorage.getItem(`sleeper_implants_${userId}`);
  return stored ? JSON.parse(stored) : [];
};

const storeImplant = (implantId: string): void => {
  const userId = getUserId();
  const imps = getStoredOwnedImplants();
  imps.push(implantId);
  localStorage.setItem(`sleeper_implants_${userId}`, JSON.stringify(imps));
};

const removeImplant = (implantId: string): void => {
  const userId = getUserId();
  const imps = getStoredOwnedImplants();
  const idx = imps.indexOf(implantId);
  if (idx !== -1) {
    imps.splice(idx, 1);
    localStorage.setItem(`sleeper_implants_${userId}`, JSON.stringify(imps));
  }
...
// Equipped slots persistence
const getStoredEquippedSlots = (): Record<number, string | null> => {
  const userId = getUserId();
  const stored = localStorage.getItem(`sleeper_equipped_slots_${userId}`);
  return stored ? JSON.parse(stored) : { 1: null, 2: null, 3: null, 4: null, 5: null };
};

const storeEquippedSlots = (slots: Record<number, string | null>): void => {
  const userId = getUserId();
  localStorage.setItem(`sleeper_equipped_slots_${userId}`, JSON.stringify(slots));
};
...
export default function App() {
  ...
  const [ghostCheckedLetters, setGhostCheckedLetters] = useState<string[]>([]);
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (desktop) {
        ...
        // Mobile/tablet: scale to fit viewport
        const widthRatio = window.innerWidth / 1920;
        const heightRatio = window.innerHeight / 1080;
        setScale(Math.min(widthRatio, heightRatio, 0.9)); // Cap at 0.9 for mobile to prevent tiny UI
      }
    };
  ...
  // Material, blueprint, and implant state
  const initialMaterials = getStoredMaterials();
  const [materials, setMaterials] = useState<Record<string, number>>(initialMaterials);
  const initialBlueprints = getStoredOwnedBlueprints();
  const [ownedBlueprints, setOwnedBlueprints] = useState<string[]>(initialBlueprints);
  const initialImplants = getStoredOwnedImplants();
  const [ownedImplants, setOwnedImplants] = useState<string[]>(initialImplants);
  const initialSlots = getStoredEquippedSlots();
  const [equippedSlots, setEquippedSlots] = useState<Record<number, string | null>>(initialSlots);
  const [showCloneBay, setShowCloneBay] = useState(false);
  const [showManufacturing, setShowManufacturing] = useState(false);
  const [manufacturingTab, setManufacturingTab] = useState<'blueprints' | 'sources'>('blueprints');
  const [selectedSlot, setSelectedSlot] = useState<1 | 2 | 3 | 4 | 5 | null>(null);
  const [marketTab, setMarketTab] = useState<'hints' | 'ships' | 'blueprints'>('hints');
  const [materialDropMessage, setMaterialDropMessage] = useState('');
  ...
  // Merge ship + implant bonuses into a single object
  const totalBonuses = (() => {
    const ship = activeShip?.bonuses || {} as Ship['bonuses'];
    const merged: Ship['bonuses'] = { ...ship };
    for (const slot of [1, 2, 3, 4, 5] as const) {
      const implantId = equippedSlots[slot];
      if (implantId) {
        const implant = IMPLANTS.find(i => i.id === implantId);
        if (implant) {
          for (const [key, value] of Object.entries(implant.bonuses)) {
            const k = key as keyof Ship['bonuses'];
            if (typeof value === 'number') {
              (merged as Record<string, unknown>)[k] = ((merged as Record<string, unknown>)[k] as number || 0) + value;
            } else if (typeof value === 'boolean') {
              (merged as Record<string, unknown>)[k] = (merged as Record<string, unknown>)[k] || value;
            }
          }
        }
      }
  ...
  const cooldownReduction = totalBonuses.boosterCooldownReduction || 0;
  ...
  const startNewGame = useCallback((diff: Difficulty = difficulty) => {
    ...
    const shipTimerBonus = totalBonuses.timerBonus || 0;
    setOxygen(settings.timer + shipTimerBonus);
    
    // Apply ship free tools at game start
    const freeAnalyzers = totalBonuses.freeDataAnalyzer || 0;
    const freeScanners = totalBonuses.freeCargoScanner || 0;
    const freeBypasses = totalBonuses.freeEmergencyBypass || 0;
    ...
    if (totalBonuses.predatorInstinct) {
    ...
    const blackOpsCharges = totalBonuses.blackOps || 0;
    setBlackOpsUses(blackOpsCharges);

    // Proteus: Ghost In Machine - set uses per game
    const ghostInMachineCharges = totalBonuses.ghostInMachine || 0;
    setGhostInMachineUses(ghostInMachineCharges);
    setShowGhostInMachine(false);
    setGhostCheckedLetters([]);
    if (ghostInputRef.current) ghostInputRef.current.value = '';
  ...
  }, [difficulty, boosterActive, boosterRounds, gamesSinceBooster, activeShip, equippedSlots]);
  ...
  const handleLoss = useCallback((msg: string) => {
    ...
    if (totalBonuses.adaptiveShield && !adaptiveShieldUsed) {
      const today = new Date().toDateString();
  ...
  const wrongGuesses = guessedLetters.filter((l: string) => !word.includes(l) && !ghostCheckedLetters.includes(l));
    ...
    setOwnedImplants(lossStored);
    setEquippedSlots({ 1: null, 2: null, 3: null, 4: null, 5: null });
    localStorage.setItem(`sleeper_implants_${lossUserId}`, JSON.stringify(lossStored));
  ...
  }, [word, guessedLetters, activeShip, adaptiveShieldUsed, ownedImplants, equippedSlots]);
  ...
  const handleWin = useCallback((baseReward: number) => {
    ...
    const shipMultiplier = totalBonuses.iskMultiplier || 1;
    // Apply ship booster multiplier bonus (Legion)
    const shipBoosterMultiplier = totalBonuses.boosterMultiplier || 0;
    ...
    const bonusLootChance = totalBonuses.bonusLootChance || 0;
    let bonusLootText = '';
    ...
    setTotalEarned((prev: number) => {
      ...
      const standingBonus = totalBonuses.standingProgressBonus || 0;
    ...
    // Material drops on successful hack
    const dropTable = MATERIAL_DROPS[difficulty];
    if (dropTable) {
      const newMats = { ...materials };
      const dropMessages: string[] = [];
      const rollDrop = (entries: { id: string; min: number; max: number }[]) => {
        for (const entry of entries) {
          const amount = Math.floor(Math.random() * (entry.max - entry.min + 1)) + entry.min;
          newMats[entry.id] = (newMats[entry.id] || 0) + amount;
          const mat = MATERIALS.find(m => m.id === entry.id);
          if (mat) dropMessages.push(`${mat.icon} ${amount}x ${mat.name}`);
        }
      };
      rollDrop(dropTable.common);
      rollDrop(dropTable.uncommon);
      if (Math.random() < 0.3) rollDrop(dropTable.rare);
      setMaterials(newMats);
      storeMaterials(newMats);
      if (dropMessages.length > 0) {
        setMaterialDropMessage(`Materials recovered: ${dropMessages.join(', ')}`);
        setShowMaterialDrop(true);
        setTimeout(() => setShowMaterialDrop(false), 4000);
      }
    }
  ...
  }, [totalEarned, boosterActive, activeShip, equippedSlots, ownedImplants, materials, difficulty]);
  ...
  useEffect(() => {
    if (oxygen <= 0 && status === 'playing' && !docked) {
      handleLoss('OXYGEN DEPLETED - FACILITY LOCKDOWN ENGAGED');
    }
  }, [oxygen, status, docked, handleLoss]);
  ...
  // Persist equipped slots to localStorage
  useEffect(() => {
    storeEquippedSlots(equippedSlots);
  }, [equippedSlots]);
  ...
  const handleGuess = (letter: string) => {
    ...
    if (totalBonuses.cloakAbility && !word.includes(letter) && cloakActive && !cloakUsed) {
      setCloakActive(false);
      setCloakUsed(true);
  ...
  const activateCloak = () => {
    if (cloakUsed || !totalBonuses.cloakAbility) return;
    setCloakActive(true);
  ...
  const checkGhostInMachine = () => {
    if (ghostInMachineUses <= 0 || status !== 'playing') return;
    const raw = ghostInputRef.current?.value?.trim().toUpperCase() || '';
    ...
    setMessage(`GHOST IN MACHINE: Letter "${letter}" is ${isPresent ? 'PRESENT' : 'NOT PRESENT'}`);
    setTimeout(() => setMessage('ENCRYPTION BREACH IN PROGRESS...'), 2000);
    if (ghostInputRef.current) ghostInputRef.current.value = '';
    setShowGhostInMachine(false);
  };
  ...
  const useShapeShifter = (newDifficulty: Difficulty) => {
    if (shapeShifterUsed || !totalBonuses.shapeShifter || status !== 'playing') return;
  ...
  const getEffectiveHintCost = () => {
    const reduction = totalBonuses.toolCostReduction || 0;
  ...
  const buySecurityConnections = () => {
    ...
    const boosterBonus = totalBonuses.boosterRoundsBonus || 0;
  ...
  const useDataAnalyzer = () => {
    ...
    const bonus = totalBonuses.dataAnalyzerBonus || 0;
  ...
  const useEmergencyBypass = () => {
    ...
    const bonus = totalBonuses.emergencyBypassBonus || 0;
    const numToReveal = 1 + bonus;
    const shuffled = [...unrevealed].sort(() => Math.random() - 0.5);
    const lettersToReveal = shuffled.slice(0, numToReveal);
    setGuessedLetters((prev: string[]) => [...prev, ...lettersToReveal]);
  ...
  const useCargoScanner = () => {
    ...
    const numToReveal = 1 + (totalBonuses.vowelScanBonus || 0);
  ...
  // Blueprint & Manufacturing functions
  const buyBlueprint = (blueprint: Blueprint) => {
    if (isk < blueprint.cost || ownedBlueprints.includes(blueprint.id)) return;
    setIsk((prev: number) => prev - blueprint.cost);
    setOwnedBlueprints((prev: string[]) => [...prev, blueprint.id]);
    storeBlueprint(blueprint.id);
  };

  const craftImplant = (blueprint: Blueprint) => {
    const implant = IMPLANTS.find(i => i.id === blueprint.implantId);
    if (!implant) return;
    if (ownedImplants.includes(implant.id)) return;
    const canCraft = blueprint.materials.every(m => (materials[m.materialId] || 0) >= m.amount);
    if (!canCraft) return;
    const newMats = { ...materials };
    for (const mat of blueprint.materials) {
      newMats[mat.materialId] = (newMats[mat.materialId] || 0) - mat.amount;
      if (newMats[mat.materialId] <= 0) delete newMats[mat.materialId];
    }
    setMaterials(newMats);
    storeMaterials(newMats);
    setOwnedImplants((prev: string[]) => [...prev, implant.id]);
    storeImplant(implant.id);
    setMessage(`MANUFACTURING COMPLETE: ${implant.name} constructed`);
    setTimeout(() => setMessage('ENCRYPTION BREACH IN PROGRESS...'), 2000);
  };

  const getMaterialCount = (materialId: string): number => materials[materialId] || 0;

  // Clone Bay functions
  const equipImplantToSlot = (slot: 1 | 2 | 3 | 4 | 5, implantId: string) => {
    const implant = IMPLANTS.find(i => i.id === implantId);
  ...
  const getImplantForSlot = (slot: number): Implant | null => {
  ...
  return (
    <div className="relative min-h-[100dvh] flex flex-col p-2 lg:p-4 xl:p-6 overflow-x-hidden overflow-y-auto no-scrollbar bg-eve-bg selection:bg-eve-accent selection:text-black">
        ...
        {showRankUp && (
          ...
          >
            <div className="bg-black/90 border-2 border-eve-accent p-8 rounded-lg shadow-[0_0_50px_rgba(0,255,255,0.3)] text-center">
              ...
              <div className="absolute top-0 left-0 w-8 h-8 border-t-2 border-l-2 border-eve-accent/30 pointer-events-none" />
              <div className="absolute bottom-0 right-0 w-8 h-8 border-b-2 border-r-2 border-eve-accent/30 pointer-events-none" />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {/* Main Layout Grid */}
      <div className="flex-1 flex flex-col lg:flex-row gap-4 lg:gap-6 z-10 overflow-y-auto lg:overflow-visible min-h-0">
        
        {/* Left Column: Tactical Overview & Loadout */}
        <div className="hidden lg:flex lg:w-1/4 flex-col gap-4 lg:gap-6 min-h-0 h-full flex-shrink-0">
          ...
          <div className="panel-border bg-eve-panel p-4 flex-1 flex flex-col justify-between">
            ...
            <div className="space-y-3">
              ...
              {/* Pacifier: Black Ops */}
              {totalBonuses.blackOps && activeShip.bonuses.blackOps > 0 && (
                <div className="flex items-center justify-between p-2 bg-black/40 border border-white/10">
                  <div className="flex items-center gap-2">
              ...
              {totalBonuses.ghostInMachine && activeShip.bonuses.ghostInMachine > 0 && (
                <div className="flex items-center justify-between p-2 bg-black/40 border border-white/10">
                  <div className="flex items-center gap-2">
                    ...
                    {!showGhostInMachine ? (
                      ...
                      <div className="flex gap-1">
                        ...
                        <button
                          ...
                          disabled={ghostInMachineUses <= 0 || status !== 'playing'}
                          className="px-2 py-1 text-[9px] uppercase bg-cyan-400/30 border border-cyan-400/50 text-cyan-400 hover:bg-cyan-400/40 transition-all disabled:opacity-20 disabled:cursor-not-allowed"
                        >
                          ✓
                        </button>
                </div>
              ...
              {showShapeShifter && totalBonuses.shapeShifter && !shapeShifterUsed && (
        ...
        <div className="w-full lg:w-1/2 flex flex-col items-start justify-start py-1 min-h-0 overflow-hidden">
          {docked ? (
            <div className="w-full flex flex-col items-center justify-center gap-4 sm:gap-6 p-4 sm:p-6 bg-black/80 rounded-lg border border-eve-accent/30 shadow-lg overflow-y-auto no-scrollbar">
              <h1 className="text-2xl sm:text-3xl font-bold mb-1 text-eve-accent">Docked at Station</h1>
              <p className="mb-2 text-sm sm:text-lg opacity-80 text-white text-center px-4">Select your ship and difficulty, then undock to begin hacking and collecting loot!</p>

              {/* Ship Selection */}
              <div className="w-full max-w-xl mx-auto mb-4">
                <div className="text-xs uppercase font-bold text-eve-accent mb-2">Select Ship</div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 p-4 border border-eve-accent/30 bg-black/40 rounded-lg max-h-[300px] overflow-y-auto custom-scrollbar">
                ...
                <div className="flex gap-2 justify-center flex-wrap">
                  {Object.entries(DIFFICULTY_SETTINGS).map(([key, settings]) => (
                    <button
                      ...
                      className={`px-3 py-2 rounded-lg text-[10px] sm:text-xs font-bold uppercase transition-all border-2 ${difficulty === key ? (key === 'highsec' ? 'bg-eve-accent text-black border-eve-accent' : key === 'streamer' ? 'bg-purple-500 text-white border-purple-500' : key === 'wormhole' ? 'bg-green-700 text-white border-green-700' : 'bg-eve-danger text-black border-eve-danger') : 'border-white/20 text-white/70 hover:border-eve-accent/50'}`}
                    >
                      {settings.label}
                    </button>
                  ))}
              ...
              {/* Clone Bay & Manufacturing Buttons */}
              <div className="w-full max-w-xl mx-auto flex gap-3 mt-2">
                <button
                  onClick={() => setShowCloneBay(true)}
                  className="flex-1 px-4 py-3 bg-emerald-700/20 border border-emerald-600/50 text-emerald-400 text-xs font-bold uppercase rounded-lg hover:bg-emerald-700/30 transition-all"
                >
                  Clone Bay
                </button>
                <button
                  onClick={() => setShowManufacturing(true)}
                  className="flex-1 px-4 py-3 bg-cyan-600/20 border border-cyan-500/50 text-cyan-400 text-xs font-bold uppercase rounded-lg hover:bg-cyan-600/30 transition-all"
                >
                  Manufacturing
                </button>
              </div>

              {/* Equipped Implants Summary */}
              <div className="w-full max-w-xl mx-auto">
                <div className="flex items-center justify-center gap-2 text-[10px] text-white/60">
                  {[1, 2, 3, 4, 5].map(slot => {
                    const imp = getImplantForSlot(slot);
                    return (
                      <div key={slot} className="flex items-center gap-1">
                        <span className="text-white/30">S{slot}:</span>
                        <span className={imp ? 'text-eve-accent' : 'text-white/20'}>
                          {imp ? imp.name : 'Empty'}
                        </span>
                        {slot < 5 && <span className="text-white/10">|</span>}
                      </div>
                    );
                  })}
                </div>
              ...
              <button
                className="px-6 py-3 sm:px-8 sm:py-4 bg-eve-accent text-black text-lg sm:text-xl font-bold rounded-lg shadow-lg hover:bg-eve-accent/80 transition-all mt-1"
            ...
            <>
              ...
              {/* Word Display */}
              <div className="flex flex-wrap justify-center gap-2 sm:gap-3 md:gap-4 my-4 w-full">
                {word.split('').map((char, i) => (
                  <div key={i} className="flex flex-col items-center justify-center w-10 sm:w-12 md:w-14">
                    <div className="flex items-center justify-center w-full h-12 sm:h-14 md:h-16">
                      <motion.span 
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        className={`text-3xl sm:text-4xl md:text-5xl font-display leading-none transition-all ${guessedLetters.includes(char) ? 'text-eve-accent' : difficulty === 'wormhole' ? 'text-transparent' : 'text-white/20'}`}
                      >
                        {guessedLetters.includes(char) ? char : '?'}
                      </motion.span>
                    </div>
                    {difficulty !== 'wormhole' && (
                      <div className={`w-full h-0.5 mt-1 rounded ${guessedLetters.includes(char) ? 'bg-eve-accent' : 'bg-white/10'}`} />
                    )}
                  </div>
                ))}
              </div>
              ...
              {message && (
                <div className="text-center my-2">
                  <motion.div 
                    ...
                    className="inline-block px-4 py-2 text-xs font-bold uppercase tracking-widest text-eve-accent bg-eve-accent/10 border border-eve-accent/30 rounded"
                  >
                    {message}
                  </motion.div>
                </div>
                    ...
                    {row.map(letter => {
                      ...
                      return (
                        <motion.button
                          ...
                          className={`
                            w-6 h-6 sm:w-8 sm:h-8 md:w-9 md:h-9 lg:w-9 lg:h-9 xl:w-12 xl:h-12 font-display text-[10px] sm:text-xs md:text-sm lg:text-sm xl:text-lg transition-all border relative overflow-hidden
        ...
        <div className="w-full lg:w-1/4 flex flex-col gap-4 lg:gap-6 min-h-0">
          {/* Clone State Panel */}
          <div className="panel-border bg-eve-panel p-4 flex-1 flex flex-col">
            <h2 className="text-xs font-bold uppercase tracking-widest mb-4 border-b border-eve-accent/20 pb-2 flex items-center gap-2">
            ...
            <div className="space-y-3">
              ...
              {/* Equipped Implants Display */}
              <div className="flex flex-col gap-1">
                <span className="text-[10px] opacity-50 uppercase">Active Implants:</span>
                <div className="flex flex-wrap gap-1">
                  {[1, 2, 3, 4, 5].map(slot => {
                    const imp = getImplantForSlot(slot);
                    return (
                      <div key={slot} className={`text-[9px] px-1.5 py-0.5 border ${imp ? 'border-purple-500/30 text-purple-400' : 'border-white/10 text-white/20'}`}>
                        S{slot}: {imp ? imp.name : 'Empty'}
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="flex flex-col gap-1">
                ...
                <span className="text-[9px] opacity-40">
                  ...
                  {totalBonuses.iskMultiplier && activeShip.bonuses.iskMultiplier > 1 && ` × ${activeShip.bonuses.iskMultiplier.toFixed(2)} Ship`}
          ...
          <div className="panel-border bg-eve-panel p-4 flex-1 flex flex-col">
            <div className="mb-4">
              ...
              <div className="flex gap-2 border-b border-eve-accent/20">
                ...
                <button
                  ...
                  className={`flex-1 py-2 text-[10px] uppercase font-bold transition-all border-b-2 ${marketTab === 'blueprints' ? 'border-eve-accent text-eve-accent' : 'border-transparent text-white/50 hover:text-white/70'}`}
                >
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar">
              ...
              <Tooltip 
                ...
                subContent={`Cost: ${effectiveBypassCost.toLocaleString()} ISK${totalBonuses.toolCostReduction || activeShip?.id === 'helios' ? ` (${Math.round((1 - (activeShip?.id === 'helios' ? 0.7 : 1 - (totalBonuses.toolCostReduction || 0))) * 100)}% ship discount)` : ''}`}
                position="left"
              >
                <button 
        ...
        {showCloneBay && (
          ...
          >
            <motion.div
              initial={{ scale: 0.9, opacity: 0, y: 30 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
              className="w-full max-w-2xl max-h-[90vh] overflow-y-auto panel-border bg-eve-panel relative"
            >
              <div className="h-1 w-full bg-emerald-700 shadow-[0_0_15px_rgba(4,120,87,0.5)]" />
              <div className="p-6 space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="font-display text-3xl uppercase tracking-tighter text-emerald-400">
                      Clone Bay
                    </h2>
                    <p className="text-[10px] opacity-50 uppercase tracking-wider">
                      Implant Socket Configuration
                    </p>
                  </div>
                  <div className="text-right">
                    <div className="text-[10px] opacity-50 uppercase">Owned Implants</div>
                    <div className="text-lg font-bold text-emerald-400">{ownedImplants.length}</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                  {([1, 2, 3, 4, 5] as const).map(slot => {
                    const equipped = getImplantForSlot(slot);
                    const available = IMPLANTS.filter(i => i.slot === slot && ownedImplants.includes(i.id));
                    const isSelected = selectedSlot === slot;
                    return (
                      <div key={slot} className="bg-black/40 border border-white/10 p-4">
                        ...
                        {equipped ? (
                          <div className="bg-emerald-600/10 border border-emerald-600/30 p-3">
                            <div className="flex items-center justify-between">
                              <div>
                                <div className="text-xs font-bold text-emerald-400">{equipped.name}</div>
                                <div className="text-[9px] text-white/60">{equipped.description}</div>
                              </div>
                              <span className="text-[8px] text-eve-success uppercase">Equipped</span>
                            </div>
                          </div>
                        ) : isSelected && (
                          <div className="space-y-2 max-h-40 overflow-y-auto">
                            {available.length === 0 ? (
                              <div className="text-[10px] text-white/30 italic py-2 text-center">
                                No implants available for this slot. Manufacture some first.
                              </div>
                            ) : available.map(imp => (
                              <button
                                key={imp.id}
                                onClick={() => {
                                  equipImplantToSlot(slot, imp.id);
                                  setSelectedSlot(null);
                                }}
                                className="w-full flex items-center justify-between p-3 border border-white/10 hover:border-emerald-600/50 bg-black/40 hover:bg-emerald-600/5 transition-all text-left"
                              >
                                <div>
                                  <div className="text-[11px] font-bold text-emerald-400">{imp.name}</div>
                                  <div className="text-[8px] text-white/50">{imp.description}</div>
                                </div>
                                <span className="text-[8px] text-eve-accent uppercase">Equip</span>
                              </button>
                            ))}
                          </div>
                        )}
                        ...
                        {!equipped && !isSelected && (
                          <div className="text-[9px] text-white/20 italic">Empty slot — Select an implant to equip</div>
                        )}
                      </div>
                    );
                  })}
        ...
        {showManufacturing && (
          ...
          >
            <motion.div
              initial={{ scale: 0.9, opacity: 0, y: 30 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
              className="w-full max-w-3xl max-h-[90vh] overflow-y-auto panel-border bg-eve-panel relative"
            >
              <div className="h-1 w-full bg-cyan-500 shadow-[0_0_15px_rgba(6,182,212,0.5)]" />
              <div className="p-6 space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    {MATERIALS.map(mat => {
                      ...
                      for (const [diff, drops] of Object.entries(MATERIAL_DROPS)) {
                        for (const rarity of ['common', 'uncommon', 'rare'] as const) {
                          if (drops[rarity].some(d => d.id === mat.id)) {
                            sources.push({ diff, label: diff.charAt(0).toUpperCase() + diff.slice(1), rarity });
                          }
                        }
                      }
                      return (
