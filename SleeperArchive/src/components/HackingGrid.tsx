      ...
      >
        ...
        {coreNode && (
          <g opacity={coreRevealed ? 1 : 0.2} style={{ transition: 'opacity 0.5s' }}>
            ...
            {/* Text */}
            <text x={coreNode.x} y={coreNode.y} textAnchor="middle" dominantBaseline="middle" fill="#000" fontSize="6" fontFamily="monospace" fontWeight="bold">CORE</text>
            {/* Pulse ring */}
          ...
          {nodes.filter(n => !n.isCore).map((node) => {
            ...
            return (
              <g key={node.id}>
                ...
                {/* Inner symbol */}
                <text x={node.x} y={node.y} textAnchor="middle" dominantBaseline="middle" fill={isExposed ? "#aaddff" : "#886655"} fontSize="6" fontFamily="monospace">
                  []
                </text>
