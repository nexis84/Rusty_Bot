(function () {
  ...
  function showMissionDetail(mission, level, pageName) {
    ...
    if (blitzText) {
      html += '<div class="detail-section detail-section--blitz"><h3>Blitz</h3><div>' + escapeHtml(blitzText) + '</div></div>';
    }

    // Pockets
    if (detail.pockets && detail.pockets.length) {
      if (infoFields.length || blitzText) {
    ...
    if (detail.tips) {
      ...
      html += '<div class="briefing-content"><div class="detail-section detail-section--tips"><div>' + escapeHtml(detail.tips) + '</div></div></div>';
      html += '<button class="briefing-toggle">Show more</button>';
      html += '</div>';
    }
    if (detail.loot) {
      if (detail.tips || (detail.pockets && detail.pockets.length)) html += '<div class="content-divider"></div>';
      html += '<div class="detail-section detail-section--loot"><h3>Loot & Bounty</h3><div>' + escapeHtml(detail.loot) + '</div></div>';
    }
  ...
  function initAnomalyControls() {
    ...
    // Wire probe scan controls
    $('#probeScanAnalyzeBtn').addEventListener('click', analyzeProbeScan);
    $('#probeScanClearBtn').addEventListener('click', clearProbeScan);
    $('#probeScanAuto').addEventListener('change', function () {
      probeScanAuto = this.checked;
    });
    $('#probeScanInput').addEventListener('input', function () {
      if (probeScanAuto && anomalyActiveTab === 'probescan') {
        analyzeProbeScan();
      }
    });
  ...
  function showAnomalyDetail(entry) {
    ...
    if (hasSurvivalData) {
      ...
      if (survival.tips) {
        html += '<div class="briefing-section">';
        html += '<div class="briefing-heading">Tips</div>';
        html += '<div class="briefing-content"><div class="detail-section detail-section--tips"><div>' + escapeHtml(survival.tips) + '</div></div></div>';
        html += '<button class="briefing-toggle">Show more</button>';
        html += '</div>';
      }
      if (survival.loot) {
        if (survival.tips) html += '<div class="content-divider"></div>';
        html += '<div class="detail-section detail-section--loot"><h3>Loot & Bounty</h3><div>' + escapeHtml(survival.loot) + '</div></div>';
      }
    }
  ...
  function showEpicArcMissionDetail(pageName) {
    ...
    if (detail.loot) {
      if (detail.tips) html += '<div class="content-divider"></div>';
      html += '<div class="detail-section detail-section--loot"><h3>Loot & Bounty</h3><div>' + escapeHtml(detail.loot) + '</div></div>';
    }
  ...
  function renderNpcLine(line) {
    ...
    if (/(^|\s)(neuts?|energy neutralizer)($|\s)/i.test(lower)) {
      return '<span class="tag tag-neuts">NEUTS</span>';
    }
    if (/(^|\s)(ecm|electronic countermeasures?)($|\s)/i.test(lower)) {
      return '<span class="tag tag-ecm">ECM</span>';
    }
    if (/(^|\s)(td|tracking disrupt(or|ion)?)($|\s)/i.test(lower)) {
      return '<span class="tag tag-td">TD</span>';
    }
    if (/(^|\s)(nos(feratu)?|energy vampire|capacitor vampire|cap drain)($|\s)/i.test(lower)) {
      return '<span class="tag tag-nos">NOS</span>';
    }
    if (lower.includes('stasis web')) {
      return '<span class="tag tag-web">WEB</span>';
    }
    if (lower.includes('warp scramble')) {
      return '<span class="tag tag-web">SCRAM</span>';
    }
  ...
  function buildAnomalyNameMap() {
    anomalyNameMap = {};
    for (const entry of anomaliesIndex) {
      if (entry.name) {
        anomalyNameMap[entry.name] = entry;
      }
    }
  }

  function clearProbeScan() {
    $('#probeScanInput').value = '';
    $('#probeScanResults').innerHTML = '';
    $('#probeScanStats').textContent = '';
  }

  function analyzeProbeScan() {
    const text = $('#probeScanInput').value;
    if (!text.trim()) {
      $('#probeScanResults').innerHTML = '<div class="probe-scan-empty">Paste probe scan results above and click Analyze.</div>';
      $('#probeScanStats').textContent = '';
      return;
    }
    const results = parseProbeScan(text);
    renderProbeScanResults(results);
  }

  function parseProbeScan(text) {
    const lines = text.split('\n').filter(l => l.trim());
    const results = [];
    ...
    for (const line of lines) {
      ...
      const pct = cols.length > 4 ? (cols[4] || '').trim() : '';
      const distance = cols.length > 5 ? (cols[5] || '').trim() : '';

      let classification = 'scanable';
      if (type === 'Cosmic Anomaly') {
        if (subtype.toLowerCase().includes('ore')) {
          classification = 'ore';
        } else {
  ...
  function groupResults(results) {
    ...
    for (const r of results) {
      ...
      if (!groups[key]) {
        groups[key] = { entries: [], key: r.match ? r.match.page : r.name || r.id };
      }
  ...
  function renderProbeScanResults(results) {
    const container = $('#probeScanResults');
    const statsEl = $('#probeScanStats');

    if (results.length === 0) {
      container.innerHTML = '<div class="probe-scan-empty">No valid probe scan data found. Make sure you paste the full contents of the probe scan window.</div>';
      statsEl.textContent = '';
      return;
    }

    const green = results.filter(r => r.classification === 'green');
    const ore = results.filter(r => r.classification === 'ore');
    const scanable = results.filter(r => r.classification === 'scanable');
    const matched = results.filter(r => r.match);
    const groups = groupResults(results);
    const totalGroups = groups.length;

    statsEl.textContent = results.length + ' site' + (results.length !== 1 ? 's' : '') +
      ' \u2014 ' + totalGroups + ' group' + (totalGroups !== 1 ? 's' : '') +
      ' \u2014 ' + green.length + ' combat, ' + ore.length + ' ore, ' + scanable.length + ' signature' + (scanable.length !== 1 ? 's' : '') +
      ' \u2014 ' + matched.length + ' have guide' + (matched.length !== 1 ? 's' : '');

    const greenGroups = groups.filter(g => g.classification === 'green');
    const oreGroups = groups.filter(g => g.classification === 'ore');
    const scanableGroups = groups.filter(g => g.classification === 'scanable');

    let html = '';

    // Combat Anomalies
    if (greenGroups.length) {
      html += '<div class="probe-scan-group">';
      html += '<div class="probe-scan-group-header">Combat Anomalies <span class="group-count">' + green.length + ' (' + greenGroups.length + ' type' + (greenGroups.length !== 1 ? 's' : '') + ')</span></div>';
      html += '<div class="probe-scan-result-list">';
      for (const g of greenGroups) {
        html += renderProbeScanGroup(g);
      }
      html += '</div></div>';
    }
    ...
    if (oreGroups.length) {
      html += '<div class="probe-scan-group">';
      html += '<div class="probe-scan-group-header">Ore Sites <span class="group-count">' + ore.length + ' (' + oreGroups.length + ' type' + (oreGroups.length !== 1 ? 's' : '') + ')</span></div>';
      html += '<div class="probe-scan-result-list">';
    ...
    // Scanable Signatures
    if (scanableGroups.length) {
      html += '<div class="probe-scan-group">';
      html += '<div class="probe-scan-group-header">Cosmic Signatures <span class="group-count">' + scanable.length + ' (' + scanableGroups.length + ' type' + (scanableGroups.length !== 1 ? 's' : '') + ')</span></div>';
      html += '<div class="probe-scan-result-list">';
      for (const g of scanableGroups) {
        html += renderProbeScanGroup(g);
      }
      html += '</div></div>';
    }
  ...
  function renderProbeScanGroup(g) {
    const pctNum = parseFloat(g.pct);
    let pctClass = '';
    if (!isNaN(pctNum)) {
      if (pctNum < 25) pctClass = 'pct-low';
      else if (pctNum < 75) pctClass = 'pct-mid';
      else pctClass = 'pct-high';
    }

    let classLabel = 'Scan';
    let classBadge = 'class-scanable';
    if (g.classification === 'green') {
      classLabel = 'Green';
      classBadge = 'class-green';
    } else if (g.classification === 'ore') {
      classLabel = 'Ore';
      classBadge = 'class-ore';
    }
    ...
    html += '<span class="probe-scan-result-pct ' + pctClass + '">' + escapeHtml(g.pct) + '</span>';
    html += '<span class="probe-scan-result-class ' + classBadge + '">' + classLabel + '</span>';
    html += '<span class="probe-scan-count' + (g.count > 1 ? '' : ' empty') + '">' + (g.count > 1 ? g.count : '') + '</span>';
    html += '<span class="' + (g.match ? 'probe-scan-result-guide' : 'probe-scan-no-match') + '">' + (g.match ? 'View Guide' : 'No guide') + '</span>';
    html += '</div>';
