...
const SECURITY_CONNECTIONS_UNLOCK_THRESHOLD = 50000000; // 50M lifetime earned to unlock
const SECURITY_CONNECTIONS_COST = 5000000; // 5M ISK - 25% multiplier boost for 5 rounds
...
export default function App() {
  ...
  }, [difficulty, boosterActive, boosterRounds, gamesSinceBooster]);
  ...
  const handleWin = useCallback((baseReward: number) => {
    ...
    const actualReward = Math.floor(baseReward * standing.multiplier * boosterMultiplier);
  ...
  const buySecurityConnections = () => {
    ...
    setIsk(prev => prev - SECURITY_CONNECTIONS_COST);
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="text-xl font-display text-white">x{(getStandingLevel(totalEarned).multiplier * (boosterActive ? 1.25 : 1)).toFixed(2)}</span>
                  {boosterActive && (
                ...
                <span className="text-[9px] opacity-40">
                  ...
                  {boosterActive && ` × 1.25 = x${(getStandingLevel(totalEarned).multiplier * 1.25).toFixed(2)}`}
                </span>
              </div>
              <div className="flex flex-col gap-1">
