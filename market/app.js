...
// Helper function to properly escape HTML entities in item names
...
// Helper function to decode HTML entities (convert &quot; back to ")
...
// Pre-populate caches from local data files
function initLocalDataCache() {
    if (typeof Systems !== 'undefined' && Array.isArray(Systems)) {
        Systems.forEach(sys => {
            AppState.systemCache[sys.id] = sys.name;
            AppState.securityCache[sys.id] = sys.securityStatus;
        });
        console.log(`Loaded ${Object.keys(AppState.systemCache).length} systems into cache`);
    }
    if (typeof Stations !== 'undefined') {
        Object.entries(Stations).forEach(([id, station]) => {
            AppState.locationCache[parseInt(id)] = station.name;
        });
        console.log(`Loaded ${Object.keys(AppState.locationCache).length} stations into cache`);
    }
    if (typeof Regions !== 'undefined') {
        Object.entries(Regions).forEach(([id, name]) => {
            AppState.regionCache[parseInt(id)] = name;
        });
        console.log(`Loaded ${Object.keys(AppState.regionCache).length} regions into cache`);
    }
}
...
async function initApp() {
    ...
    // Pre-populate caches from local data
    initLocalDataCache();
    ...
    console.log('Systems available:', typeof Systems !== 'undefined');
    console.log('Stations available:', typeof Stations !== 'undefined');
    console.log('Regions available:', typeof Regions !== 'undefined');
    ...
    function renderGroupNode(group) {
        ...
        if (Array.isArray(group.items) && group.items.length > 0) {
            ...
            sortedItems.forEach(item => {
                ...
                leaf.dataset.name = decodeHtml(item.name);
                leaf.innerHTML = `<span>${escapeHtml(item.name)}</span>`;
                leaf.addEventListener('click', () => loadItem(item.id, decodeHtml(item.name)));
...
async function renderPopularItems(forceRefresh = false) {
    ...
    items.forEach(item => {
        ...
        const isSkin = item.name.toUpperCase().includes('SKIN');
        let iconHtml;
        if (isBlueprint) {
            iconHtml = '<div class="blueprint-icon"><i class="fas fa-scroll"></i></div>';
        } else if (isSkin) {
            iconHtml = '<div class="skin-icon"><i class="fas fa-palette"></i></div>';
        } else {
            iconHtml = `<img src="https://images.evetech.net/types/${item.id}/icon?size=64" alt="${item.name}" class="item-icon" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'" /><div class="skin-icon" style="display:none"><i class="fas fa-palette"></i></div>`;
}
        card.innerHTML = `
            ${iconHtml}
...
function renderHomeFavorites() {
    ...
    AppState.favorites.forEach(item => {
        ...
        card.innerHTML = `
            ${iconHtml}
            <div class="item-name">${escapeHtml(item.name)}</div>
            <div class="item-category">${escapeHtml(item.category || 'Item')}</div>
        `;
        card.addEventListener('click', () => loadItem(item.id, item.name));
        container.appendChild(card);
    });
...
function handleSearchInput() {
    ...
    suggestions.innerHTML = scored.map(item => {
        return `
            <div class="suggestion-item ${item.score >= 1000 ? 'exact-match' : ''}" data-type-id="${item.id}" data-name="${decodeHtml(item.name)}">
                <div class="suggestion-text">
                    <span class="suggestion-name">${escapeHtml(item.name)}</span>
                    <span class="suggestion-type">${escapeHtml(item.path[0])}</span>
                </div>
            </div>
        `;
    }).join('');
...
function switchTab(tabName) {
    ...
    // Load skins data when skins tab is selected
    if (tabName === 'skins' && AppState.currentItem) {
        loadSkinsData(AppState.currentItem.id, AppState.currentItem.name);
}
...
async function loadItem(typeId, name, forceRefresh = false) {
    ...
    try {
        ...
        // Update state
        AppState.currentItem = { id: typeId, name: decodeHtml(name) };
        ...
        const isSkinItem = name.toUpperCase().includes('SKIN');
        if (isBlueprintItem) {
            itemIconEl.innerHTML = `<div class="blueprint-icon-large"><i class="fas fa-scroll"></i></div>`;
        } else if (isSkinItem) {
            itemIconEl.innerHTML = `<div class="skin-icon-large"><i class="fas fa-palette"></i></div>`;
        } else {
            itemIconEl.innerHTML = `<img src="https://images.evetech.net/types/${typeId}/icon?size=128" alt="${name}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'" /><div class="skin-icon-large" style="display:none"><i class="fas fa-palette"></i></div>`;
}
...
async function getAllRegionIds() {
    ...
    try {
        ...
        // Fallback to all regions from local data if fetch fails
        if (typeof Regions !== 'undefined') {
            return Object.keys(Regions).map(Number).filter(id => id < 11000000);
        }
        return [10000002, 10000043, 10000032, 10000030, 10000042, 10001004];
}
}
...
// Get system security status (cached, pre-populated from local data)
async function getSystemSecurity(systemId) {
    // Check cache first (pre-populated at startup from local data)
...
async function resolveLocation(locationId, elementIdOrCallback) {
    ...
    // Check local station data first
    if (typeof Stations !== 'undefined' && Stations[locationId]) {
        name = Stations[locationId].name;
    ...
    try {
        // NPC station IDs are in range 60,000,000–64,000,000.
        // Player Upwell structure IDs are >= 10^12 — the /stations/ endpoint
        // returns 400 for these, so skip it entirely and go straight to fallback.
        const isPlayerStructure = locationId >= 1000000000000;
        
        if (!isPlayerStructure) {
            // Try NPC station endpoint
            const url = `${ESI_BASE}/universe/stations/${locationId}/`;
            const r = await fetch(url);
        ...
        // Player Upwell structures (>= 10^12) require authentication for
        // /universe/structures/{id}/ — skip that call for public market data.
...
async function resolveLocationWithSystem(locationId, systemId, callback) {
    ...
    try {
        if (!locationName) {
            // NPC station IDs are in range 60,000,000–64,000,000.
            // Player Upwell structure IDs are >= 10^12 — the /stations/ endpoint
            // returns 400 for these, so skip it entirely.
            const isPlayerStructure = locationId >= 1000000000000;
        ...
        // Player Upwell structures (>= 10^12) require auth for /universe/structures/;
        // skip that call — fall through to the system-name fallback below.
        
        // If location couldn't be resolved but we have a system, fetch system name
        if (!locationName && systemId) {
            // Check system cache first (pre-populated from local data)
...
// Load skins data for a ship
async function loadSkinsData(shipId, shipName) {
    const container = el('skinsInfo');
    if (!container) return;
    
    container.innerHTML = '<p class="loading-text">Loading skins data...</p>';
    
    try {
        // Find skins that contain "SKIN" and the ship name
        // We match words to avoid "Omen" matching "Omen Navy Issue" incorrectly when it shouldn't, 
        // actually standard string inclusion is mostly fine but let's be careful.
        // Usually ship skins are named "[ShipName] [SkinName] SKIN".
        const skins = AppState.allItems.filter(item => {
            const lowerName = item.name.toLowerCase();
            return lowerName.includes('skin') && 
                   lowerName.includes(shipName.toLowerCase());
        });
        ...
        if (skins.length === 0) {
            container.innerHTML = `
                <div class="no-manufacturing">
                    ...
                    <p>No skins found for ${shipName}.</p>
                </div>
            `;
            return;
}
        ...
        let html = '<div class="skins-section" style="padding: 15px;">';
        html += `<h4><i class="fas fa-paint-brush"></i> Available Skins for ${shipName}</h4>`;
        html += '<div class="item-grid" style="margin-top: 15px;">';
        
        for (const skin of skins) {
            const escapedName = skin.name.replace(/'/g, "\\'").replace(/"/g, '&quot;');
            html += `
                <div class="item-card" onclick="loadItem(${skin.id}, '${escapedName}')" style="cursor: pointer;">
                    <div class="skin-icon"><i class="fas fa-palette"></i></div>
                    <div class="item-name">${escapeHtml(skin.name)}</div>
                    <div class="item-category">SKIN</div>
                </div>
            `;
}
