...
const SubCategories = {
    ...
    blueprints: {
        groups: [
            ...
            // 6. Module blueprints (weapons, defense, propulsion)
            { id: "module_blueprints", name: "Module Blueprints", filter: (item) => item.name.toLowerCase().includes('blueprint') && (item.name.toLowerCase().includes('laser') || item.name.toLowerCase().includes('blaster') || item.name.toLowerCase().includes('railgun') || item.name.toLowerCase().includes('autocannon') || item.name.toLowerCase().includes('artillery') || item.name.toLowerCase().includes('missile launcher') || item.name.toLowerCase().includes('turret') || item.name.toLowerCase().includes('afterburner') || item.name.toLowerCase().includes('microwarpdrive') || item.name.toLowerCase().includes('shield') || item.name.toLowerCase().includes('armor') || item.name.toLowerCase().includes('repairer') || item.name.toLowerCase().includes('harden') || item.name.toLowerCase().includes('plating') || item.name.toLowerCase().includes('ecm') || item.name.toLowerCase().includes('sensor') || item.name.toLowerCase().includes('warp') || item.name.toLowerCase().includes('capacitor') || item.name.toLowerCase().includes('power diagnostic') || item.name.toLowerCase().includes('reactor control') || item.name.toLowerCase().includes('rig') || item.name.toLowerCase().includes('mining laser') || item.name.toLowerCase().includes('strip miner') || item.name.toLowerCase().includes('tractor beam') || item.name.toLowerCase().includes('salvager') || item.name.toLowerCase().includes('smartbomb') || item.name.toLowerCase().includes('vampire') || item.name.toLowerCase().includes('neutralizer')) },
            // 7. Ship blueprints (catch-all for ship hulls - won't match above filters)
            { id: "ship_blueprints", name: "Ship Blueprints", filter: (item) => item.name.toLowerCase().includes('blueprint') && item.name.toLowerCase().endsWith('blueprint') },
