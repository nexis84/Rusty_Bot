...
// ---------- Jump graph ----------
// Stargate adjacency for jump-distance searches (Finder tab).
// --jumps-only emits just PI/pi-jumps.js and leaves the big data files alone.
const JUMPS_ONLY = process.argv.includes('--jumps-only');

function buildJumpAdjacency() {
  const gates = loadLines('mapStargates.jsonl');
  const adj = new Map();
  for (const g of gates) {
    const from = g.solarSystemID;
    const to = g.destination && g.destination.solarSystemID;
  ...
  const out = {};
  for (const [id, set] of adj) out[id] = [...set].sort((a, b) => a - b);
  return out;
}

function writeJumpsFile() {
  const jumps = buildJumpAdjacency();
  const JUMPS_OUT = path.join(ROOT, 'PI', 'pi-jumps.js');
  fs.writeFileSync(JUMPS_OUT,
    '// Generated by scripts/build-pi-data.js — do not edit by hand\n' +
    'const PI_JUMPS = ' + JSON.stringify(jumps) + ';\n' +
    'if (typeof module !== "undefined" && module.exports) module.exports = PI_JUMPS;\n');
  const gateCount = Object.values(jumps).reduce((n, a) => n + a.length, 0) / 2;
  console.log('pi-jumps.js:', Object.keys(jumps).length, 'systems |', gateCount, 'gates |',
    (fs.statSync(JUMPS_OUT).size / 1024).toFixed(1), 'KB');
}
...
try {
  const metaLine = fs.readFileSync(path.join(SDE, '_sde.jsonl'), 'utf8').split('\n').filter(Boolean)[0];
  sdeBuild = String(JSON.parse(metaLine).buildNumber);
} catch (e) {
  console.warn('Could not read _sde.jsonl build number:', e.message);
}
const meta = {
  sdeBuild,
