...
// Planet radius lookup (planet id -> km), used for PI link CPU/Powergrid cost.
// ESI does not expose planet radius, so this ships with the SDE build and is
// loaded lazily alongside the systems data. mapPlanets.radius is in metres.
const planetRadii = {};
for (const p of planetsRaw) {
  if (typeof p.radius === 'number') planetRadii[p._key] = Math.round(p.radius / 1000);
}
const PLANETS_OUT = path.join(ROOT, 'PI', 'pi-planets.js');
fs.writeFileSync(PLANETS_OUT, '// Generated by scripts/build-pi-data.js — do not edit by hand\n' +
  'const PI_PLANET_RADII = ' + JSON.stringify(planetRadii) + ';\n' +
  'if (typeof module !== "undefined" && module.exports) module.exports = PI_PLANET_RADII;\n');
